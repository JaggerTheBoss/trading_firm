...
from .. import db
from ..models import User, Account, Referral
import logging
...
def buy_account():
    ...
    for _ in range(quantity):
        account = Account(user_id=user_id, size=size, balance=size, highest_balance=size, mll=mlls[size], contract_limit=contracts[size], status='express')
...
def reset_account(account_id):
    ...
    account.status = 'express'  # Back to Express, not live yet
