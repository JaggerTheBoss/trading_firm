...
def get_stats(account_id):
    ...
    daily_data = db.session.query(db.func.date(Trade.timestamp), db.func.sum(Trade.pnl), db.func.count(Trade.id))\
        .filter_by(account_id=account_id).group_by(db.func.date(Trade.timestamp)).order_by(db.func.date(Trade.timestamp).desc()).all()
    ...
    stats = {
        ...
        "daily_stats": [{"date": str(date), "pnl": float(pnl or 0), "trades": trades} for date, pnl, trades in daily_data]
    }
